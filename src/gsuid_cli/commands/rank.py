...
def _crown_compute_items(details: list[dict[str, object]]) -> list[dict[str, object]]:
    skill_map = _json_map("avatarId2SkillList_mapping_7.0.0.json")
...
def _character_id(query: str) -> str:
    ...
    for avatar_id, name in _json_map("avatarId2Name_mapping_7.0.0.json").items():
        if needle == _normalize(str(name)):
            return avatar_id
    for name, avatar_id in _json_map("enName2AvatarID_mapping_7.0.0.json").items():
        if needle == _normalize(str(name)):
...
def _character_name(character_id: str) -> str:
    return str(_json_map("avatarId2Name_mapping_7.0.0.json").get(str(character_id)) or character_id)
...
def _total_star_count(star: str) -> int:
    return sum(
        1 for value in _json_map("avatarId2Star_mapping_7.0.0.json").values() if str(value) == star
    )
...
def _total_weapon_count(star: str) -> int:
    return sum(
        1
        for item in _json_map("weaponList_7.0.0.json").values()
