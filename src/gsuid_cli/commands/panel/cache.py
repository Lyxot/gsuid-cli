...
def avatar_name(avatar: JsonDict) -> str:
    mapped = _mapped_text("avatarId2Name_mapping_6.7.0.json", avatar_id(avatar))
    if mapped:
        return mapped
    value = (
        avatar.get("name")
...
def avatar_aliases(avatar: JsonDict) -> list[str]:
    values = [
        avatar_id(avatar),
        _mapped_text("avatarId2Name_mapping_6.7.0.json", avatar_id(avatar)),
        avatar.get("name"),
        avatar.get("route"),
...
def artifact_list(avatar: JsonDict) -> list[JsonDict]:
    ...
    for equip in _list_of_dicts(avatar.get("equipList")):
        ...
        mapped_name = _mapped_text("icon2Name_mapping_6.7.0.json", flat.get("icon"))
        name = mapped_name or flat.get("name") or ""
        artifact = {
            "item_id": str(equip.get("itemId") or equip.get("id") or ""),
            "name": str(name or equip.get("name") or ""),
            "slot": flat.get("equipType") or equip.get("slot"),
            "set_name": _mapped_text("artifact2attr_mapping_6.7.0.json", name)
            or flat.get("setName")
...
def _weapon_name(equip: JsonDict, flat: JsonDict) -> str:
    return str(
        _mapped_text("weaponId2Name_mapping_6.7.0.json", equip.get("itemId") or equip.get("id"))
        or _mapped_text("weaponHash2Name_mapping_6.7.0.json", flat.get("nameTextMapHash"))
...
def _weapon_type(equip: JsonDict, flat: JsonDict) -> str | None:
    ...
    if isinstance(weapon_data, dict):
        ...
        if value:
            return str(value)
    return _mapped_text("weaponHash2Type_mapping_6.7.0.json", flat.get("nameTextMapHash"))
...
def _weapon_list() -> dict[str, object]:
    return _text_map("weaponList_6.7.0.json")
