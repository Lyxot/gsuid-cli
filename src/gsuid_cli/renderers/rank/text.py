...
def _character_name(character: Mapping[str, object]) -> str:
    ...
    mapped = _text_map("avatarId2Name_mapping_7.0.0.json").get(avatar_id)
...
def _weapon_name(weapon: Mapping[str, object]) -> str:
    ...
    if name == "-":
        return name
    for item in _text_map("weaponList_7.0.0.json").values():
...
def _artifact_set_label(name: object, item: Mapping[str, object]) -> str:
    ...
    artifact_name = _text_map("icon2Name_mapping_7.0.0.json").get(icon)
    if artifact_name:
        set_name = _text_map("artifact2attr_mapping_7.0.0.json").get(str(artifact_name))
        if set_name:
...
def _artifact_name(artifact: Mapping[str, object]) -> str:
    ...
    mapped = _text_map("icon2Name_mapping_7.0.0.json").get(icon_key)
