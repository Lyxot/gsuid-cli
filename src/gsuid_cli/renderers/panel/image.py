...
def _avatar_star(avatar: Mapping[str, object], panel: Mapping[str, object]) -> int:
    mapped = _map("avatarId2Star_mapping_6.7.0.json").get(_avatar_id(avatar, panel))
...
def _total_avatar_star(star: str) -> int:
    return sum(
        1 for value in _map("avatarId2Star_mapping_6.7.0.json").values() if str(value) == star
    )


def _total_weapon_star(star: str) -> int:
    weapon_list = _map("weaponList_6.7.0.json")
    count = 0
...
def _weapon_name(weapon: Mapping[str, object], panel: Mapping[str, object]) -> str | None:
    flat = _flat(weapon)
    mapped = _map("weaponHash2Name_mapping_6.7.0.json").get(str(flat.get("nameTextMapHash") or ""))
    if mapped:
        return str(mapped)
...
def _weapon_type(weapon: Mapping[str, object]) -> str:
    mapped = _map("weaponHash2Type_mapping_6.7.0.json").get(
...
def _artifact_name(
    ...
    mapped = _map("icon2Name_mapping_6.7.0.json").get(icon_name or "")
...
def _skill_entries(avatar: Mapping[str, object]) -> list[dict[str, object]]:
    ...
    if not isinstance(skill_map, Mapping):
        return []
    icons = _nested_map("skillId2Name_mapping_6.7.0.json", "Icon")
    entries = []
...
def _talent_icon_urls(avatar: Mapping[str, object]) -> list[str]:
    ...
    if not isinstance(talents, list):
        return []
    icons = _nested_map("talentId2Name_mapping_6.7.0.json", "Icon")
    urls = []
...
def _avatar_element(avatar: Mapping[str, object], panel: Mapping[str, object]) -> str:
    mapped = _map("avatarName2Element_mapping_6.7.0.json").get(_avatar_name(avatar, panel) or "")
...
def _avatar_name(avatar: Mapping[str, object], panel: Mapping[str, object]) -> str | None:
    ...
    if value:
        return value
    mapped = _map("avatarId2Name_mapping_6.7.0.json").get(_avatar_id(avatar, panel))
