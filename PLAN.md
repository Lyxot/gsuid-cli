...
- Status: completed.
- Result: added agent-focused README quickstart, generated command reference,
  credential safety guidance, docs drift test, CI workflow, release build
  artifact workflow, and source distribution manifest entries for docs/scripts.
- Command reference: `docs/commands.md` is generated by
  `scripts/generate_command_reference.py` from capability metadata and checked
  by tests/CI.
- Release prep: CI runs tests, ruff, format check, command-reference check, and
  package build. Release workflow builds distributions on tags or manual
  dispatch and uploads artifacts without publishing.
- Verification:
  - `.venv/bin/python scripts/generate_command_reference.py --check`
  - `.venv/bin/python -m pytest tests/test_docs.py tests/test_meta_commands.py`
  - `.venv/bin/python -m pytest`
  - `.venv/bin/ruff check .`
  - `.venv/bin/ruff format --check .`
  - `.venv/bin/python -m build`
  - standalone review agent before commit; invalid manual QR example was fixed
    and reverified.
