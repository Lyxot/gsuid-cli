...
CLI 文本语言会优先读取 `GSUID_LANG` / `GSUID_LANGUAGE`，再读取 `LANGUAGE` / `LC_ALL` / `LC_MESSAGES` / `LANG`，最后检测系统语言；macOS 会读取系统首选语言。目前支持 `zh-CN` 与 `en`；无法识别时默认使用中文。
可以在 `$GSUID_HOME/config.toml` 中配置全局默认行为；优先级为命令行参数 > 环境变量 > 配置文件 > 内置默认值。
